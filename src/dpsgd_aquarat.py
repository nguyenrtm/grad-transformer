...
if accelerator.is_main_process:
    with torch.no_grad():
        ...
        print(f"Step 0 aquarat: {result}")
        accelerator.log({f"aquarat_{k}": v for k, v in result.items()}, step=0)
        accelerator.log({"eval_loss": eval_loss}, step=0)
        accelerator.log({f"aquarat_records_table": records_table}, step=0)
        ...
        for batch in memory_safe_data_loader:
            ...
            if step >= num_training_steps or stop_training and step != 0:
                ...
                if accelerator.is_main_process:
                    ...
                    with torch.no_grad():
                        result, records_table = evaluate_model(model, val_dataloader)
                        print(f"Step {step} aquarat: {result}")
                        accelerator.log({f"aquarat_{k}": v for k, v in result.items()}, step=step)
                        accelerator.log({f"aquarat_records_table": records_table}, step=step)
