...
from utils import generate_and_tokenize_prompt, Prompter
...
if args.save_fname:
    ...
    if args.split:
        save_fname = f"{safe_model_path}_{safe_dataset_path}_{args.tune_option}_split_{args.split}_{args.split_option}_seed_{args.seed}"
...
if args.split:
    if args.split_option == 'w2s':
        train_dataset = train_dataset.select(range(0, int(len(train_dataset)*args.split)))
        print(f"W2S: Keep the first {args.split*100}% of the training set")
    else:
...
train_dataloader = DataLoader(tokenized_train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=data_collator)
val_dataloader = DataLoader(tokenized_val_dataset, batch_size=4, shuffle=False, collate_fn=default_data_collator)
val_dataloader_for_loss = DataLoader(tokenized_val_dataset_for_loss, batch_size=4, shuffle=False, collate_fn=data_collator)
...
initial_save_path = os.path.join(f"models/{safe_dataset_path}/{save_fname}", "lora_initial.pt")
...
if accelerator.is_main_process:
    with torch.no_grad():
        ...
        print(f"Step 0 dialogsum: {result}")
        accelerator.log({f"dialogsum_{k}": v for k, v in result.items()}, step=0)
        accelerator.log({"eval_loss": eval_loss}, step=0)
        accelerator.log({f"dialogsum_records_table": records_table}, step=0)
        ...
        with accelerator.accumulate(model):
            ...
            if step >= num_training_steps or stop_training and step != 0:
                ...
                if accelerator.is_main_process:
                    ...
                    with torch.no_grad():
                        result, records_table = evaluate_model(model, val_dataloader)
                        print(f"Step {step} dialogsum: {result}")
                        accelerator.log({f"dialogsum_{k}": v for k, v in result.items()}, step=step)
                        accelerator.log({f"dialogsum_records_table": records_table}, step=step)

                        unwrapped_model = accelerator.unwrap_model(model)
                        unwrapped_model.save_pretrained(
                            f"models/{safe_dataset_path}/{save_fname}",
                        ...
                        delta_save_path = os.path.join(f"models/{safe_dataset_path}/{save_fname}", "final_lora_gradients.pt")
                        os.makedirs(os.path.dirname(delta_save_path), exist_ok=True)
                        torch.save(delta_lora_grads, delta_save_path)

                        print(f"Latest model saved at step {step} to models/{safe_dataset_path}/{save_fname}/final_lora_gradients.pt; {result}")

                        if result['rouge1'] > best_metric:
                            best_metric = result['rouge1']
                            delta_save_path = os.path.join(f"models/{safe_dataset_path}/{save_fname}", "best_lora_gradients.pt")
                            os.makedirs(os.path.dirname(delta_save_path), exist_ok=True)
                            torch.save(delta_lora_grads, delta_save_path)
                            print(f"Best model saved at step {step} to models/{safe_dataset_path}/{save_fname}/best_lora_gradients.pt; {result}")
