...
if accelerator.is_main_process:
    with torch.no_grad():
        ...
        print(f"Step 0 CommonsenseQA: {result}")
        accelerator.log({f"CommonsenseQA_{k}": v for k, v in result.items()}, step=0)
        accelerator.log({"eval_loss": eval_loss}, step=0)
        accelerator.log({f"CommonsenseQA_records_table": records_table}, step=0)
...
while step <= num_training_steps:
    ...
    for batch in train_dataloader:
        with accelerator.accumulate(model):
            ...
            if step >= num_training_steps or stop_training == True and step != 0:
                stop_training = True
                if accelerator.is_main_process:
                    model.eval()
                    with torch.no_grad():
                        result, records_table = evaluate_model(model, val_dataloader)
                        print(f"Step {step} CommonsenseQA: {result}")
                        accelerator.log({f"CommonsenseQA_{k}": v for k, v in result.items()}, step=step)
                        accelerator.log({f"CommonsenseQA_records_table": records_table}, step=step)
