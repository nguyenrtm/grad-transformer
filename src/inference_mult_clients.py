...
from src.convert_gradients import group_by_layer_and_merge, split_tensor_to_lora, parse_key
from src.eval_math import *
...
def load_dpsgd(path):
    gradient = torch.load(path, weights_only=True)
    new_gradient = {}
    for k, v in gradient.items():
        name = k.replace("_module.", "")
        new_gradient[name] = v
    return new_gradient
...
def dp_noise_gradients(lora_gradients, noise_option, clipping_threshold=2.5, delta=1e-5, eps=10., device="cuda"):
    ...
    print(grad_norm.item(), clipping_threshold, clipping_scale)
...
def avg_gradients(lora_gradients_lst):
    avg_grads = {}
    param_lst = lora_gradients_lst[0].keys()
    for param in param_lst:
        for i in range(len(lora_gradients_lst)):
            if 'layers.27.self_attn.q_proj.lora_A' in param:
                print(f"{i} {param}: {lora_gradients_lst[i][param][0]}")
            if i == 0:
                avg = lora_gradients_lst[i][param]
            else:
                avg += lora_gradients_lst[i][param]
        avg_grads[param] = avg / len(lora_gradients_lst)
        if 'layers.27.self_attn.q_proj.lora_A' in param:
            print("Averaged:")
            print(avg_grads[param][0])
    
    return avg_grads

def sum_gradients(lora_gradients_lst):
    sum_grads = {}
    param_lst = lora_gradients_lst[0].keys()
    for param in param_lst:
        for i in range(len(lora_gradients_lst)):
            if i == 0:
                s = lora_gradients_lst[i][param]
            else:
                s += lora_gradients_lst[i][param]
        sum_grads[param] = s
...
if __name__ == "__main__":
    ...
    small_model = get_peft_model(small_model, small_lora_config)
    ...
    if args.init_small_path and args.init_large_path:
        small_model = init_weights(small_model, load_dpsgd(args.init_small_path))
        large_model = init_weights(large_model, load_dpsgd(args.init_large_path))

    tokenizer = AutoTokenizer.from_pretrained(args.small_model_path, padding_side='left')

    predicted_large_lora_gradients_lst = []
    small_lora_gradients_lst = []

    for i in range(len(args.small_model_gradient_path)):
        print(f"Using gradients from: {args.small_model_gradient_path[i]}")
        small_lora_gradients = load_dpsgd(args.small_model_gradient_path[i])
        if args.dpsgd:
            small_lora_gradients = dp_noise_gradients(
                small_lora_gradients, 
        ...
        small_lora_gradients_lst.append(small_lora_gradients)

    avg_small_lora_gradients = avg_gradients(copy.deepcopy(small_lora_gradients_lst))

    small_lora_gradients = avg_gradients(copy.deepcopy(small_lora_gradients_lst))
    large_lora_keys = [name for name, param in large_model.named_parameters() if param.requires_grad]

    small_gradients = group_by_layer_and_merge(sorted(small_lora_gradients.items(), key=lambda x: parse_key(x[0])), model_name=args.small_model_path, print_layers=False)
    ...
    if args.target_large_gradient_path:
        large_model.eval()
        target_large_gradients = torch.load(args.target_large_gradient_path, weights_only=True)
        target_large_model = apply_gradients(copy.deepcopy(large_model), target_large_gradients)
    ...
    with torch.no_grad():
        ...
        if args.idx != None:
            i = args.idx
            with open(args.indices_path[i], 'rb') as file:
                idx = pickle.load(file)

            val_dataset = train_dataset_tmp.select(idx['test'])
            ...
            if 'aqua_rat' in args.dataset_path.lower():
                ...
                val_dataset = val_dataset.map(dataset.format_dialogsum, batched=True)
            elif 'samsum' in args.dataset_path.lower():
                val_dataset = val_dataset.map(dataset.format_samsum, batched=True)
            ...
            target_small_model = apply_gradients(copy.deepcopy(small_model), avg_small_lora_gradients)
            result = evaluate_model(target_small_model, val_dataloader, args.dataset_path, args.ans_template, args.ref_template)
            print(f"Small model after fine-tuning: {result}")

            if args.target_large_gradient_path:
                result = evaluate_model(target_large_model, val_dataloader, args.dataset_path, args.ans_template, args.ref_template)
                print(f"Large model after fine-tuning: {result}")

            result = evaluate_model(predicted_large_model, val_dataloader, args.dataset_path, args.ans_template, args.ref_template)
            print(f"Predicted aggregated large model: {result}")