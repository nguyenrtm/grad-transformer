...
if accelerator.is_main_process:
    with torch.no_grad():
        ...
        print(f"Step 0 drop: {result}")
        accelerator.log({f"drop_{k}": v for k, v in result.items()}, step=0)
        accelerator.log({"eval_loss": eval_loss}, step=0)
        accelerator.log({f"drop_records_table": records_table}, step=0)
        ...
        with accelerator.accumulate(model):
            ...
            if step >= num_training_steps or stop_training and step != 0:
                ...
                if accelerator.is_main_process:
                    ...
                    with torch.no_grad():
                        result, records_table = evaluate_model(model, val_dataloader)
                        print(f"Step {step} drop: {result}")
                        accelerator.log({f"drop_{k}": v for k, v in result.items()}, step=step)
                        accelerator.log({f"drop_records_table": records_table}, step=step)
